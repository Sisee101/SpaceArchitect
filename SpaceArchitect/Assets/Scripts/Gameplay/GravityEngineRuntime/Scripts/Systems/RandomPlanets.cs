...
public class RandomPlanets : MonoBehaviour {
	...
	void Start () {
		...
		foreach (GameObject g in planetPrefabs) {
			...
			if (g.GetComponent<OrbitUniversal>() == null) {
				Debug.LogError("Prefab " + g.name + " missing component OrbitUniversal");
				return;
			}
		}
	...
	private void AddBody(int num) {
        ...
        planet.name = "random." + num;

		OrbitUniversal ou = planet.GetComponent<OrbitUniversal>();
		ou.centerNbody = gameObject.GetComponent<NBody>();
		// set ranges with appropriate limits
		ou.p = Random.Range(Mathf.Max(a_min, 0.1f), a_max);
        ou.eccentricity = Random.Range(Mathf.Max(ecc_min, 0f), Mathf.Min(0.99f, ecc_max));
        ou.inclination = Random.Range(Mathf.Max(0, incl_min), Mathf.Min(180f, incl_max));
        ou.omega_lc = Random.Range(Mathf.Max(0, omega_lc_min), Mathf.Min(359.9f, omega_lc_max));
        ou.omega_uc = Random.Range(Mathf.Max(0, omega_uc_min), Mathf.Min(359.9f, omega_uc_max));
        ou.phase = Random.Range(Mathf.Max(0, phase_min), Mathf.Min(359.9f, phase_max));
