...
public class AsteroidData {
	...
	public static void SetSolarBody(SolarBody sbody, string line) {
		try {
			...
			sbody.epoch = SolarUtils.ConvertMJDtoAD(float.Parse(data[0], CultureInfo.InvariantCulture));
			sbody.a = float.Parse(data[1], CultureInfo.InvariantCulture);
			sbody.ecc = float.Parse(data[2], CultureInfo.InvariantCulture);
			sbody.inclination = float.Parse(data[3], CultureInfo.InvariantCulture);
			sbody.omega_lc = float.Parse(data[4], CultureInfo.InvariantCulture);
			sbody.omega_uc = float.Parse(data[5], CultureInfo.InvariantCulture);
			// TODO: Check this
    ...
    public static void SetOrbit(float epoch, OrbitUniversal orbitU, SolarBody sb) {
        ...
        float period = SolarUtils.GetPeriodYears(sb);
        float numPeriod = deltaYears / period;
        orbitU.phase = OrbitEllipse.MeanToTrueAnomoly(NUtils.DegreesMod360(sb.longitude + 360f * numPeriod), (float) orbitU.eccentricity);
    }
