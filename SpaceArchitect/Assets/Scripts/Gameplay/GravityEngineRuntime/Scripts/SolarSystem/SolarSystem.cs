...
///
/// The attributes of the planets can be altered (for "what if" experiments) by editing the 
...
public class SolarSystem : MonoBehaviour {
	...
	public void UpdateEpochTime(float newTime) {
        ...
        foreach (OrbitUniversal orbitU in OrbitUniversalInSystem()) {
            SolarBody sbody = orbitU.transform.gameObject.GetComponent<SolarBody>();
            if (sbody != null) {
    ...
    private List<OrbitUniversal> OrbitUniversalInSystem() {
        List<OrbitUniversal> orbits = new List<OrbitUniversal>();
        NBody[] nbodies = GetComponentsInChildren<NBody>();
        foreach (NBody nbody in nbodies) {
            OrbitUniversal orbit = nbody.transform.gameObject.GetComponent<OrbitUniversal>();
            if (orbit != null) {
                orbits.Add(orbit);
            }
        }
    ...
    private void SetPhase(OrbitUniversal orbitU, SolarBody sbody) {
        sbody.SetEllipseForEpoch(_epochTime, orbitU);
    }
	...
	public void InitOrbit(SolarBody sbody) {
        // prefer orbitU but support ellipse for backwards compatibility
