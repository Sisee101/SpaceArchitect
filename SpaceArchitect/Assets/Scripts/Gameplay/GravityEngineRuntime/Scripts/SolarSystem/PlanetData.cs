...
public class PlanetData {
    ...
    /// <param name="epoch">Epoch.</param>
    /// <param name="ellipseBase">Ellipse base.</param>
    public static void SetOrbit(float epoch, OrbitUniversal orbitU, SolarBody sb) {
        Debug.LogFormat("set orbit {0}: epoch={1}", orbitU.gameObject.name, epoch);
        // Based on JPL data, apply the per century drift to the orbital parameters
        // T = number of centuries past J2000
        float T = (epoch - 2000f) / 100f;
        orbitU.eccentricity = sb.ecc + sb.ecc_dot * T;
        double a = sb.a + sb.a_dot * T;
        orbitU.p = a * (1 - orbitU.eccentricity * orbitU.eccentricity);
        orbitU.inclination = sb.inclination + sb.inclination_dot * T;
        orbitU.omega_uc = NUtils.DegreesMod360(sb.omega_uc + sb.omega_uc_dot * T);
        // following JPL doc (http://ssd.jpl.nasa.gov/txt/aprx_pos_planets.pdf)
        double current_omega_bar = sb.omega_lc_bar + sb.omega_lc_bar_dot * T;
        orbitU.omega_lc = NUtils.DegreesMod360(current_omega_bar - orbitU.omega_uc);
        double L = sb.longitude + sb.longitudeDot * T;
        double mean_anomoly = NUtils.DegreesMod360(L - current_omega_bar);
        orbitU.phase = mean_anomoly;
    }
    ...
    /// <param name="epoch">Epoch.</param>
    /// <param name="ellipseBase">Ellipse base.</param>
    public static void SetOrbitJD(double julianDate, OrbitUniversal orbitU, SolarBody sb)
    {
        // Based on JPL data, apply the per century drift to the orbital parameters
        // T = number of centuries past J2000
