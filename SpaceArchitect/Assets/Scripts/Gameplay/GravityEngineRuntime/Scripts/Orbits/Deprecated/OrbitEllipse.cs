...
/// <summary>
/// DEPREACTED: Please use OrbitUniversal
...
//[Obsolete("Please use OrbitUniversal instead. This will be removed in a future release.")]
public class OrbitEllipse : EllipseBase, INbodyInit, IFixedOrbit, IOrbitScalable {
	...
	public void InitNBody(float physicalScale, float massScale) {
        ...
        if (evolveMode == evolveType.KEPLERS_EQN) {
            if (!Application.isPlaying) {
                // if in the editor/DrawGizmos all we care about is position
	...
	public void Evolve (double physicsTime,  GravityState gs, ref double[] r_new, ref double[] v_new, bool doCallbacks = true) {
        ...
        gs.GetVelocityDouble(centerNbody, ref v_center);
        velocityScaled = ellipse_orientation * v_xy + new Vector3((float) v_center[0], (float)v_center[1], (float)v_center[2]);
        v_new[0] = velocityScaled.x;
        v_new[1] = velocityScaled.y;
        v_new[2] = velocityScaled.z;
