...
public class OrbitSegment: MonoBehaviour  {
	...
	void Awake () {
        ...
        if (centerBody == null) {
            Debug.LogError("No center body for OrbitSegment");
        }
    ...
    public void SetCenterObject(GameObject newCenterBody) {
        ...
        // Needed for calling SetCenterObject before Awake    
        if( orbitU == null )
		      orbitU = transform.gameObject.AddComponent<OrbitUniversal>();
        orbitU.SetOrbitPredictor(true);
        aroundNBody = centerBody.GetComponent<NBody>();
    ...
    void Update () {
        ...
        Vector3d centerPos = GravityEngine.Instance().GetPositionDoubleV3(aroundNBody);

        // Is the resulting orbit an ellipse or hyperbola?
        ...
        if (destination != null) {
            ...
            destPoint = ge.UnmapFromScene(destination.transform.position);
        }
        ...
        if (orbitU.eccentricity < 1.0) {
            ...
            positions = orbitU.EllipseSegmentProRetro(numPoints, centerPos.ToVector3(), pos.ToVector3(), destPoint, retrograde);
        } else {
            float radius = (pos.ToVector3() - centerPos.ToVector3()).magnitude;
