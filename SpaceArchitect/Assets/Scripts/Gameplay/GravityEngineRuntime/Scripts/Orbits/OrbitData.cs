...
public class OrbitData {
                     ...
                     float phase)
    {
        ...
        this.ecc = e;
        this.inclination = i;
        this.omega_uc = omega_uc; 
        this.omega_lc = omega_lc;
        this.phase = phase;
    }
    ...
    /// 
    /// This is not used with the OrbitUniversal attached to the NBody, that only provides the initial orbit 
    /// info, not the live orbit info.
    /// </summary>
    /// <param name="orbitU"></param>
    public OrbitData(OrbitUniversal orbitU) {
        // if OP optimization is on, need to force an update
        OrbitPredictor op = orbitU.GetComponent<OrbitPredictor>();
        if (op != null)
            op.UpdateOrbitU();
        ...
        centralMass = orbitU.GetCenterNBody();
    ...
    public void SetOrbitForVelocity(NBody forNbody, NBody aroundNBody, Vector3 velocity) {
        ...
        // Needed for Lagrange in 4.1
        mu = (float)(ge.GetMass(centralMass) + ge.GetMass(forNbody));
        RVtoCOEWrapper(aroundNBody, r_for, velocity);
        //Debug.LogFormat("SetOrbitForVelocity {0}: a={1} perih={2} e={3} i={4} Omega={5} omega={6} r={7} V={8} mu={9}",
        //                forNbody.gameObject.name,
        //                a, perihelion, ecc, inclination, omega_uc, omega_lc, r_for, velocity, mu);
    }
    ...
    public void SetOrbitForVelocity(NBody forNbody, NBody aroundNBody) {
        nbody = forNbody;
        centralMass = aroundNBody;
        SetOrbitForVelocity(forNbody, aroundNBody, GravityEngine.instance.GetVelocity(forNbody));
    }
    ...
    public void SetOrbit(NBody forNbody, NBody aroundNBody) {
        nbody = forNbody;
        centralMass = aroundNBody;
        // BUG here: phase in OrbitU is not the curent phase. Either need to fix that, or just give up on this 
        // optimization and recalc COE each time. 
    ...
    {
        return (ecc < OrbitUtils.ecc_circular);
    }
    ...
    public Vector3d GetAxis() {
        GravityEngine ge = GravityEngine.Instance();
        Vector3 r = GetPhysicsPosition(0, relativePos: true);
        Vector3 v = GetPhysicsVelocity(0);
        return new Vector3d(Vector3.Cross(r, v).normalized);
    }
    ...
    /// <summary>
    /// Convenience method to get the phase assuming the orbit is circular. Useful for e.g. HohmannGeneral algorithms
    /// that assume the orbit is a perfect circle. 
    /// </summary>
    /// <returns></returns>
    public float GetCircPhase()
    {
        return (phase + omega_lc + omega_uc) % 360.0f;
    }

    private void RVtoCOEWrapper(NBody aroundNBody, Vector3 r, Vector3 v) {

        // XZ handled in RVtoCOE
    ...
    public Vector3 GetPhysicsPositionforEllipse(float phaseDeg) {
        ...
        return GetPhysicsPositionforEllipse(phaseDeg, relativePos: false); 
    }
    ...
    public Vector3 GetPhysicsPositionforEllipseRelative(float phaseDeg) {
        ...
        return GetPhysicsPositionforEllipse(phaseDeg, relativePos: true);
    }
    ...
    private Vector3 GetPhysicsPositionforEllipse(float phaseDeg, bool relativePos) {
        return GetPhysicsPosition(phaseDeg, relativePos);
    }
    ...
    /// <param name="relativePos">Return the position relative to the center object, otherwise absolute wolrd value.</param>
    /// <returns></returns>
    private Vector3 GetPhysicsPosition(float phaseDeg, bool relativePos)
    {
        ...
        float r = p / (1f + ecc * Mathf.Cos(phaseRad));
    ...
    public float GetPhaseDegForRadius(double radius)
    {
        ...
        if (Mathd.Abs(ecc2 - 1f) < 1E-6)
            ecc2 = (float)(1 + 1E-5);
        float theta = 0;
        if (ecc2 > 1E-8) {
            double thetaEqn = (p / radius - 1) / ecc2;
            theta = (float)Mathd.Acos(Mathd.Clamp(thetaEqn, -1.0, 1.0));
#pragma warning disable 162     // disable unreachable code warning
            if (GravityEngine.DEBUG) {
                if (Mathd.Abs(thetaEqn) > 1.0)
                    Debug.LogFormat("PHASE CLAMP: p={0} r={1} e={2}", p, radius, ecc2);
            }
#pragma warning restore 162        // apply an impulse to the indicated NBody
    ...
    public Vector3 GetPhysicsVelocityForEllipse(float phaseDeg) {
        return GetPhysicsVelocity(phaseDeg);
    }
    ...
    /// <param name="phaseDeg"></param>
    /// <returns></returns>
    public Vector3 GetPhysicsVelocity(float phaseDeg)
    {
        ...
        Vector3 v0 = ellipse_orientation * new Vector3(vx, vy, 0);
        if (GravityEngine.Instance().xzOrbits) {
    ...
    private float ComputeP()
    {
        ...
        if (ecc < 1.0) {
            ...
            // this is assuming a is positive
            p = a * (ecc * ecc - 1);
        }
        return p;
    }
