...
public class NBody : MonoBehaviour, IComparer<NBody> {
    ...
    // GE 5.1: Start to evolve to double based initial position from OrbitUniversal
    // If initWithDouble is set, then use the double position in GE SetupOneGameObject(). 
    // MUST still set initialPhyPos to ensure backwards compatibility
    // (this is not awesome, but does preserve compatibility)
    public bool initWithDouble = false;
    public Vector3d initialPhysPositionV3;
    public Vector3d vel_physV3; 
    ...
    private Object opaqueData { get; set; }
    ...
    public void SetPosVel3d(Vector3d pos, Vector3d vel)
    {
        initialPhysPositionV3 = pos;
        vel_physV3 = vel;
        initialPhysPosition = pos.ToVector3();
        vel_phys = vel.ToVector3();
        initWithDouble = true;
    }
    ...
    public bool IsFixedOrbit() {
		if (engineRef == null) {
            Debug.LogError("NBody not active: " + gameObject.name);
		}
        return 
    ...
    public void InitPosition(GravityEngine ge) {
        ...
        if (engineRef != null) {
			...
			if (initNbody != null) {
                ...
                // binary pair? (Legacy, BinaryOrbit is preferred in 5.1+)
                ...
                if (ge.units != GravityScaler.Units.DIMENSIONLESS) { 
                    ...
                    initialPhysPosition = ge.lengthScale * initialPos;
                } else {
                    // value is taken from the transform object, since Orbit in OrbitU etc. will have applied scale
...
#pragma warning disable 162        // disable unreachable code warning
        if (GravityEngine.DEBUG && Application.IsPlaying(this)) {
            Debug.Log(string.Format("Nbody {0} InitPosition: initialPos={1} initialPhyPos={2}",
                gameObject.name, initialPos, initialPhysPosition));
        }
#pragma warning restore 162        // enable unreachable code warning
    ...
    public void ApplyScale(float lengthScale, float velocityScale) {
        // this is stomped by InitNBody 
		initialPhysPosition = lengthScale * initialPos;
        if (GravityEngine.Instance().units != GravityScaler.Units.DIMENSIONLESS) {
            transform.position = initialPos * lengthScale;
        }
		vel_phys = vel * velocityScale;
		#pragma warning disable 162		// disable unreachable code warning
			if (GravityEngine.DEBUG) {
				Debug.Log(string.Format("Nbody scale: {0} xform=[{1} {2} {3}] v=[{4} {5} {6}] initialPhyPos={7} initialPos={8}",
    ...
    public void CalcOrbitDepth() {
        ...
        do {
            ...
            OrbitUniversal[] orbitU = go.GetComponents<OrbitUniversal>();
            OrbitHyper hyper = go.GetComponent<OrbitHyper>();
            BinaryPair bp = go.GetComponent<BinaryPair>();
            NBody nbody_parent = null;
            if (go.transform.parent != null)
                nbody_parent = go.transform.parent.gameObject.GetComponent<NBody>();

            if (ellipse != null) {
                go = ellipse.centerObject;
                orbitDepth++;
            } else if (orbitU.Length > 0) {
                foreach (OrbitUniversal ou in orbitU) {
                    if (ou is BinaryOrbit) {
            ...
            if (++loopCnt > LOOP_LIMIT) {
                Debug.LogError("Orbit depth too large - BUG?");
                return;
            }
        } while (go != null);
    }
