...
//	private float outOfBoundsSquared; 
	...
	*/
    ...
    void Start()
    {
        ...
        gravityEngine.AddGEStartCallback(ParticleSetup);
    }
...
//		outOfBoundsSquared = gravityEngine.outOfBounds * gravityEngine.outOfBounds; 
	...
	private void PreEvolve(int fromP, int toP, int numBodies, GravityState.NbodyState[] nbodyState) {
		...
		for (int i=0; i < numBodies; i++) {	
			if (nbodyState[i].isActive) {
				for (int j=fromP; j < toP; j++) {
					rji[0] = r[j, 0] - nbodyState[i].r_x;
					rji[1] = r[j, 1] - nbodyState[i].r_y;
					rji[2] = r[j, 2] - nbodyState[i].r_z;
					r2 = 0; 
					for (int k=0; k < GravityState.NDIM; k++) {
						r2 += rji[k] * rji[k]; 
					}
					...
					if (debugLogs && j== 0) {
						Debug.Log (string.Format ("PreEvolve : Initial a={0} {1} {2} rji={3} {4} {5} body={6} m={7}", 
							a[0,0], a[0,1], a[0,2], rji[0], rji[1], rji[2], i, nbodyState[i].m));	
					}
	...
	void ParticleLifeCycleHandler (int numBodies, GravityState.NbodyState[] nbodyState)
	{
		...
		if (lastParticleCount < particleCount) {
			...
			PreEvolve (lastParticleCount, gravityParticles.particleCount, numBodies, nbodyState);
		...
		for (int i = 0; i < gravityParticles.particleCount; i++) {
			if (seed[i] != particles[i].randomSeed) {
				...
				if (particleBySeed.ContainsKey (particles[i].randomSeed)) {
					...
					PreEvolve (i, i + 1, numBodies, nbodyState);
					particleBySeed[particles[i].randomSeed] = i;
					#pragma warning disable 162		// disable unreachable code warning
					if (debugLogs)
	...
	public double Evolve(double evolveTime, int numBodies, GravityState gravityState ) {
		...
		ParticleLifeCycleHandler (numBodies, nbodyState);
		//
				...
				if (nbodyState[i].isActive) {
					for (int j=0; j < particleCount; j++) {
						...
						if (!inactive[j]) {
							...
							if (allowRemoval && r2 < nbodyState[i].size2) {
								...
								if (debugLogs) {
									Debug.Log (string.Format ("Inactivate particle {0} size2={1} due to object at {2} {3} {4} r2={5} count={6} p={7} {8}",
								                  j, nbodyState[i].size2,
												  nbodyState[i].r_x, nbodyState[i].r_y, nbodyState[i].r_z,
												  r2, inactiveCount, r[j,0], r[j,1]));
								}
								#pragma warning restore 162
								if (oneTimeBurst) {
									r[j,0] = OUT_OF_VIEW;
							...
							r3 = r2 * System.Math.Sqrt(r2) + EPSILON; 
							a[j,0] -= nbodyState[i].m * rji[0]/r3;
							a[j,1] -= nbodyState[i].m * rji[1]/r3;
							a[j,2] -= nbodyState[i].m * rji[2]/r3;
						} // for j
					} // info
				} // for i
			}
				...
				if (nbodyState[i].isActive) {
					for (int j=0; j < particleCount; j++) {
						...
						if (!inactive[j]) {
							...
							double[] aForce = force.CalcAccelerationIPerParticle(rji, i, nbodyState);
							a[j,0] -= nbodyState[i].m * aForce[0];
							a[j,1] -= nbodyState[i].m * aForce[1];
							a[j,2] -= nbodyState[i].m * aForce[2];
						} // for j
					} // info
				} // for i
			}
	...
	public void UpdateParticles(float physicalScale, GravityEngine ge) {
		...
		float itime = ge.GetPhysTimeForInterpolation();
		for (int i=0; i < lastParticleCount; i++) {
			Vector3 position = new Vector3((float)r[i, 0], (float)r[i, 1], (float)r[i, 2]);
			if (ge.updateMode != GravityEngine.UpdateMode.FIXED_UPDATE) {
				Vector3 v1 = new Vector3((float)v[i, 0], (float)v[i, 1], (float)v[i, 2]);
				position += itime * v1;
			}
			if (inactive[i]) {
				particles[i].remainingLifetime = 0;
			} else {
				particles[i].position = ge.MapToScene(position * physicalScale);
			}
		}
	...
	public void AdjustVelocity(Vector3d velAdjust)
	{
		for (int i = 0; i < lastParticleCount; i++) {
			v[i, 0] += velAdjust.x;
			v[i, 1] += velAdjust.y;
