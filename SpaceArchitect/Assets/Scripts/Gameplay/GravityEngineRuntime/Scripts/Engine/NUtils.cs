...
public class NUtils  {
    ...
    public static double GetEnergy(int numBodies, GravityState.NbodyState[] nb)
    {
        // If any bodies have gone from active -> inactive this won't be meaningful
        double epot = 0.0;
        double ekin = 0.0;
        ...
        for (int i = 0; i < numBodies; i++) {
            for (int j = i + 1; j < numBodies; j++) {
                ...
                rji[0] = nb[j].r_y - nb[i].r_y;
                rji[0] = nb[j].r_z - nb[i].r_z;
                r2 = 0;
                for (int k = 0; k < NDIM; k++) {
                    r2 += rji[k] * rji[k];
                }
                epot -= nb[i].m * nb[j].m / System.Math.Sqrt(r2);
            }
            ekin += 0.5 * nb[i].m * nb[i].v_x * nb[i].v_x;
            ekin += 0.5 * nb[i].m * nb[i].v_y * nb[i].v_y;
            ekin += 0.5 * nb[i].m * nb[i].v_z * nb[i].v_z;
        }
        return ekin + epot;
    }
    ...
    /// <returns>The mod360.</returns>
    /// <param name="angle">Angle.</param>
    public static double DegreesMod360(double angle) {
    ...
    /// <param name="v"></param>
    /// <returns></returns>
    public static bool Array1DNaN(double[] a)
    {
    ...
    public static float AngleFullCircleRadians(Vector3 from, Vector3 to, Vector3 normal) {
    ...
    // Determine angle in radians between the vectors and express in (0, 2 Pi) 
    public static double AngleFullCircleRadians(Vector3d from, Vector3d to, Vector3d normal)
    {
        double angle = Vector3d.Angle(from, to) * GEMath.DEG2RAD;
        // check cross product wrt to normal
        Vector3d fromXto = Vector3d.Cross(from, to).normalized;
        double crossCheck = Vector3d.Dot(fromXto, normal.normalized);
        if (crossCheck < 0) {
            angle = 2f * Mathf.PI - angle;
        }
        return angle;
