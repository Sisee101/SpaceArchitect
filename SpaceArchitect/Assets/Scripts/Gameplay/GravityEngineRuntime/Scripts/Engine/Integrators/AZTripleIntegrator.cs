...
/// 
/// The key algorithm detail is the use of mathematical regularization to handle close
/// encounters where the use of 1/r^2 would result in numerical instability. The "trick"
/// is to use a change of variables to u=1/r for the two interacting bodies. 
/// 
/// The integrator has been extended to handle additional bodies by using Leapfrog to evolve
/// the additional bodies subject to the gravity of the AZT controlled bodies. Adding more than 
/// three massive bodies will result in inaccurate modelling, since first three bodies added 
/// will not be affected by the fourth. 
/// 
/// </summary>
...
//		int jc = -1; 
	...
	/// <summary>
	/// The code AZT algorithm handles three massive bodies. Anything else added to the scene needs to make use of the masses
	/// positions. As a result the first three massive bodies added are assigned to the AZT integrator and the rest go into 
	/// a Leapfrog integrator that treats the AZT bodies as fixed. 
	/// 
	/// This requires a mapping that indicates which 3 NbodyState entries were the ones added to the AZT integrator.
	/// </summary>
	private LeapfrogIntegrator leapfrogIntegrator;
	private List<int> nbodyIndexForAZT; 
...
//		jc = -1;
		...
		leapfrogIntegrator = new LeapfrogIntegrator(force: null);
		leapfrogIntegrator.Setup(maxBodies, timeStep);
		nbodyIndexForAZT = new List<int>();
	...
	public INBodyIntegrator DeepClone() {
		...
		azt.leapfrogIntegrator = (LeapfrogIntegrator) leapfrogIntegrator.DeepClone();
		azt.nbodyIndexForAZT = new List<int>(nbodyIndexForAZT);
		return azt;
	}
	...
	public void AddNBody( int bodyNum, NBody nbody, GravityState.NbodyState[] nbodyStates) {
		...
		numBodies++;
		if ((nbody.mass != 0) && (nbodyIndexForAZT.Count < 3)) {
			// AZT integrator (one of three nbodies that can be added)
			nbodyIndexForAZT.Add(bodyNum);
			nbodyStates[bodyNum].isFixed = true; // very kludgy, but mark as fixed so LF will not evolve it

			float physToWorldFactor = GravityEngine.Instance().physToWorldFactor;
			...
			if (nbody.initWithDouble) {
				Vector3d position = nbody.initialPhysPositionV3 / physToWorldFactor;
				x[1, numBodies] = position.x;
				x[2, numBodies] = position.y;
				x[3, numBodies] = position.z;
				xdot[1, numBodies] = nbody.vel_physV3.x;
				xdot[2, numBodies] = nbody.vel_physV3.y;
				xdot[3, numBodies] = nbody.vel_physV3.z;
			} else {
				Vector3 position = nbody.initialPhysPosition / physToWorldFactor;
				x[1, numBodies] = position.x;
				x[2, numBodies] = position.y;
				x[3, numBodies] = position.z;
				xdot[1, numBodies] = nbody.vel_phys.x;
				xdot[2, numBodies] = nbody.vel_phys.y;
				xdot[3, numBodies] = nbody.vel_phys.z;
			}
			name[numBodies] = numBodies;
			m[numBodies] = nbody.mass;
		}
		// add all to LF. 
		leapfrogIntegrator.AddNBody(bodyNum, nbody, nbodyStates);
	...
	public void PreEvolve(GravityState gravityState) {
		...
		GravityState.NbodyState[] nbodyState = gravityState.GetNbodyStates();

		// This integrator does not support flags. 
		for(int i=0; i < 3; i++) {
			if (!nbodyState[i].isActive) {
				Debug.LogError("AZTIntegrator does not  inactive flags");
				return;
			}
		}
		double[,] r_arg = new double[3, 3];
		for (int i=0; i < 3; i++) {
			r_arg[i, 0] = nbodyState[nbodyIndexForAZT[i] ].r_x;
			r_arg[i, 1] = nbodyState[nbodyIndexForAZT[i]].r_y;
			r_arg[i, 2] = nbodyState[nbodyIndexForAZT[i]].r_z;
		}
		AdjustToCMFrame();
...
//				r_arg[i-1,k-1] += velCM[k] * time;				
			...
			// r_arg is in order already
			nbodyState[nbodyIndexForAZT[i-1]].r_x = r_arg[i-1, 0];
			nbodyState[nbodyIndexForAZT[i - 1]].r_y = r_arg[i-1, 1];
			nbodyState[nbodyIndexForAZT[i - 1]].r_z = r_arg[i-1, 2];
		}
		if (numBodies > 3) {
			leapfrogIntegrator.PreEvolve(gravityState);
		}
	} // setup
	...
	public double Evolve(double evolveTime, GravityState gravityState, bool exactTime = false, bool reversing = false) {
...
//*       Velocity of escaper w.r. to M(3) (scaled by binary velocity).
		...
		if (numBodies > 3) {
			leapfrogIntegrator.Evolve(evolveTime, gravityState, exactTime: false, reversing: false);
		}
