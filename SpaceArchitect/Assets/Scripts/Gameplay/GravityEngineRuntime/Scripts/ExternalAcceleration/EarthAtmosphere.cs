...
public class EarthAtmosphere : MonoBehaviour, GEExternalAcceleration {
    ...
    void Start () {
        ...
        geDistanceToKm = 1 / GravityEngine.Instance().lengthScale;
    ...
    public double[] acceleration(double time, GravityState gravityState, ref double massKg) {
        ...
        if (double.IsNaN(h)) {
            Debug.LogError("h is NaN. t=" + time);
            return accel;
        }
        // Due to triangulation a ship lauching can start at a slight negative h. Only want impact
        // when go from +ve h to -ve h
        if ((h < 0) && (h_last > 0)) {
            if (isLiveState) {
                impact = true;
                Debug.LogFormat("Impact of {0} at h={1} shipPos={2} t={3}", gameObject.name, h, shipPos, time);
