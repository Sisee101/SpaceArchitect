...
public class SolarSystemEditor : Editor {
	...
	public override void OnInspectorGUI()
	{
        ...
        if (GUILayout.Button("Add All Planets")) {
            AddAllOfType(solar, PlanetData.GetNames(), SolarSystem.Type.PLANET, PlanetData.SetSolarBody);
        }

        if (GUILayout.Button("Add All Asteroids")) {
            AddAllOfType(solar, AsteroidData.GetNames(), SolarSystem.Type.ASTEROID, AsteroidData.SetSolarBody);
        }

        if (GUILayout.Button("Add All Comets")) {
            AddAllOfType(solar, CometData.GetNames(), SolarSystem.Type.COMET, CometData.SetSolarBody);
        }
    ...
    // Code from anon contributor. 
    private void AddAllOfType(SolarSystem solarSystem, string[] names, SolarSystem.Type type, Action<SolarBody, int> setSolarBody)
    {
        var tempGameObject = new GameObject("temp to hold sbody");
        tempGameObject.hideFlags = HideFlags.HideAndDontSave;
        var sbody = tempGameObject.AddComponent<SolarBody>();
        for (var i = 0; i < names.Length; i++) {
            setSolarBody(sbody, i);
            var go = solarSystem.AddObject(type, sbody);
            if (go != null) {
