...
public class MultiStageRocketEditor : Editor
{
    ...
    public override void OnInspectorGUI() {
        ...
        // Show active stage when running
        if (Application.isPlaying) {
            EditorGUILayout.LabelField(string.Format("Active Stage: {0}", mstage.activeStage));
            EditorGUILayout.LabelField(string.Format("Active Fuel: {0}", mstage.GetFuel()));
        }
