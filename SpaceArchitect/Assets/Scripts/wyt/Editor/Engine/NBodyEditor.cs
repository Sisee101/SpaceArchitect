...
public class NBodyEditor : Editor {
    ...
    public override void OnInspectorGUI()
	{
        ...
        } else if (nbody.transform.gameObject.GetComponent<BinaryOrbit>() != null) {
            controlledBy = "Generated by BinaryOrbit for the center of mass of the pair.";
        ...
        if (Application.isPlaying && (nbody.engineRef != null)) {
			// show |r|
			EditorGUILayout.LabelField(string.Format("phyPos |r|={0}", ge.GetPhysicsPosition(nbody).magnitude));
            // show the velocity in the inspector if nbody is active
            Vector3 vel = ge.GetVelocity(nbody);
            EditorGUILayout.LabelField(string.Format("Runtime velocity (GE) ={0}", vel));
            EditorGUILayout.LabelField(string.Format("                 |v| ={0}", vel.magnitude));
            if (ge.units != GravityScaler.Units.DIMENSIONLESS) {
                vel = ge.GetScaledVelocity(nbody);
                EditorGUILayout.LabelField(string.Format("Scaled velocity ={0}", vel));
                EditorGUILayout.LabelField(string.Format("                 |v| ={0}", vel.magnitude));
            }
        }
		...
		if (GUI.changed) {
			...
			nbody.vel_physV3 = new Vector3d(nbody.vel);
			nbody.size = size;
