...
public class TransformEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
		...
		if ( (t.gameObject.GetComponent<BinaryPair>() != null) ||
			(t.gameObject.GetComponent<BinaryOrbit>() != null) ||
        ...
        // NOT the usual GE, just if GE is attached to this transform!
        ...
        } else if (t.gameObject.GetComponent<BinaryOrbit>() != null) {
            // could be an OP for putting this binary in Orbit, in which case position is not an option
            OrbitUniversal[] orbits = t.GetComponents<OrbitUniversal>();
            if (orbits.Length == 1) {
                position = EditorGUILayout.Vector3Field("Position", t.localPosition);
                EditorGUILayout.LabelField("Binary Orbit detected.");
                EditorGUILayout.LabelField("Orientation controlled by orbit parameters.");
            } else {
		...
		else if (t.gameObject.GetComponent<IFixedOrbit>() != null) {
			EditorGUILayout.LabelField("Position taken from IFixedOrbit script.");
			showPosition = true;
