...
public class LambertBattin : OrbitTransfer
{
    ...
    private void LambHodograph
        ...
        // eps = 1.0e-8;  // -14
        // For more extreme length scales (ORBITAL with lengthScale = 0.001 Unity units per km) this bounds check was causing issues.
        eps = 1.0e-6;  // -14
   ...
   -----------------------------------------------------------------------------*/
            ...
            ) 
    {
        ...
        if (loops < 30) {
            ...
            LambHodograph(p, ecc, dnu, dtsec); // results put in globals v1t and v2t
        ...
        if (error == 0) {
            if (checkPlanetRadius) {
                bool hit = OrbitUtils.CheckHitPlanet(planetRadius, mu, r1, v1t,
                                                        r2,v2t,
                                                        nrev: 0);
                if (hit) {
                    error = IMPACT;
            ...
            maneuvers.Clear();
            Vector3 v1_vec = v1t.ToVector3();
            Vector3 v2_vec = v2t.ToVector3();
            Vector3d centerVel = GravityEngine.instance.GetVelocityDoubleV3(centerBody);
            Vector3d fromVelRelative = GravityEngine.instance.GetVelocityDoubleV3(fromBody) - centerVel;
            // Departure
            Maneuver departure = new Maneuver();
            departure.mtype = Maneuver.Mtype.setv;
            departure.label = "LamB.1";
            ...
            departure.railsXferMode = Maneuver.RailsTransferMode.NONE;
            // Check for SGP4 ship transfer
            OrbitUniversal shipOrbit = fromBody.GetComponent<OrbitUniversal>();
            bool railsXfer = false;
            if (shipOrbit != null) {
                railsXfer = (shipOrbit.evolveMode == OrbitUniversal.EvolveMode.SGP4_PROPAGATOR) ||
                                (shipOrbit.evolveMode == OrbitUniversal.EvolveMode.PKEPLER_J2);
                if ((shipOrbit != null) && railsXfer) {
                    departure.railsXferMode = Maneuver.RailsTransferMode.KEPLER;
                }
            }
            ...
            // Arrival (will not be required if intercept, UNLESS SGP4/PKEPLER)
            // setv is wrt to centerVel
            bool intercept = (mode == ComputeMode.INTERCEPT);
            if (((toOrbit != null) && !intercept) || railsXfer) {
                Maneuver arrival = new Maneuver();
                arrival.label = "LamB.2";
                ...
                if (intercept) {
                    arrival.mtype = Maneuver.Mtype.none;
                } else if (mode == ComputeMode.RENDEZVOUS) {
                ...
                if (intercept) {
                    // for intercept and KSeq need to specify relativeVel at arrival
                    arrival.velChange = v3d1_xfer.ToVector3();
                    arrival.relativeVel = new Vector3d(arrival.velChange);
                }

                // Check for SGP4 ship transfer
                if ((shipOrbit != null) && railsXfer) {
                    if (shipOrbit.evolveMode == OrbitUniversal.EvolveMode.SGP4_PROPAGATOR)
                        arrival.railsXferMode = Maneuver.RailsTransferMode.SGP4;
                    else
                        arrival.railsXferMode = Maneuver.RailsTransferMode.PKEPLER;
                }
    ...
    bool rendezvous)
    {
        ...
        GravityEngine ge = GravityEngine.Instance();
        OrbitUniversal targetOrbit = toOrbit.nbody.GetComponent<OrbitUniversal>();
        KeplerSequence kSeq = toOrbit.nbody.GetComponent<KeplerSequence>();
        // If on rails, can just use Evolve directly on the target
        double[] r_t = new double[] { 0, 0, 0 };
        double[] v_t = new double[] { 0, 0, 0 };
        GravityState gs = ge.GetWorldState();
        double t = ge.GetPhysicalTime() + dtsec;
       
        if (kSeq != null) {
            kSeq.Evolve(t, gs, ref r_t, ref v_t, isQuery:true);
            r2 = new Vector3d(ref r_t) - ge.GetPositionDoubleV3(fromOrbit.centralMass);
            targetVelocity = new Vector3d(ref v_t) - ge.GetVelocityDoubleV3(fromOrbit.centralMass);
        } else if ((targetOrbit!=null) && targetOrbit.IsOnRails()) {
            // sgp4 that decays may cause issues here, but would anyway
            targetOrbit.Evolve(t, gs, ref r_t, ref v_t, isQuery:true);
            r2 = new Vector3d(ref r_t) - ge.GetPositionDoubleV3(fromOrbit.centralMass);
            targetVelocity = new Vector3d(ref v_t) - ge.GetVelocityDoubleV3(fromOrbit.centralMass);
        } else {
            NBody target = toOrbit.nbody;
            Vector3d r = ge.GetPositionDoubleV3(target) - ge.GetPositionDoubleV3(fromOrbit.centralMass);
            Vector3d v = ge.GetVelocityDoubleV3(target) - ge.GetVelocityDoubleV3(fromOrbit.centralMass);
            OrbitPropagator prop = new OrbitPropagator(r, v, time0: 0.0, toOrbit.mu);
            (Vector3d r_new, Vector3d v_new) = prop.PropagateToTime(dtsec);
            r2 = r_new;
            targetVelocity = v_new;
        }
