...
public class ShipInfo : MonoBehaviour {
    ...
    void Start () {
        ...
        Vector3 shipPos = ge.GetScenePosition(ship);
        initial_altitude = (pos - ge.GetScenePosition(planet)).magnitude - (float)atmosphere.heightEarthSurface;
    ...
    public void SetTextInfo(float fuel, Vector3 angles)
    {
        GravityEngine ge = GravityEngine.Instance();
        time.text = string.Format(time_format, ge.GetTimeWorldSeconds() , ge.GetTimeZoom() );
        Vector3 pos = ge.GetScenePosition(ship);
        Vector3 vel = ge.GetScaledVelocity(ship);
        position.text = string.Format(pos_format, pos.x, pos.y, pos.z);
        velocity.text = string.Format(vel_format, vel.x, vel.y, vel.z);
        fuelText.text = string.Format(fuel_format, fuel);
        attitude.text = string.Format(attitude_format, angles.y, angles.x, angles.z);

        altitudeNow = (pos - ge.GetScenePosition(planet)).magnitude - (float) atmosphere.heightEarthSurface; 
