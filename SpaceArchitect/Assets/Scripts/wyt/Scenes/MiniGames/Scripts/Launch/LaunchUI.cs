...
{
    [Header("SPACE to launch, WASD to steer.")]
    [Header("F1/F2 for camera select")]
    ...
    private const float LAUNCH_LOCKOUT = 60f;
        ...
        if (Input.GetKeyUp(KeyCode.T)) {
         ...
         if (running) {
            ...
            if ((altitude < 0) && (time > LAUNCH_LOCKOUT)) {
                // ship has crashed
                DisplayManager.Instance().DisplayMessage("Impact t=" + time);
