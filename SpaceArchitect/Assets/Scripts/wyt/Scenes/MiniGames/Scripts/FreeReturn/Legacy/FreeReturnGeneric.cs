...
public class FreeReturnGeneric : MonoBehaviour
{
    ...
    private void AddGhostBodies() {
        ...
        for (int i = 0; i < NUM_GHOST_SHIPS; i++) {
            ...
            ghostShipOrbit[i].p_inspector = soiRadius/ge.lengthScale;  // inspector needs unscaled value
            ghostShipOrbit[i].centerNbody = planet;
            ghostShipOrbit[i].evolveMode = OrbitUniversal.EvolveMode.KEPLERS_EQN;
        ...
        ghostShipOrbit[TLI].p_inspector = shipOrbit.p_inspector;
        ...
        ghostShipOrbit[TO_MOON].p_inspector = shipOrbit.p_inspector;
        ...
        // don't need ENTER_SOI predictor, will use the one from SOI_HYPER to show the orbit
    ...
    private void ComputeTransfer() {
        ...
        // scaling ok to here
        //double velFactor = GravityScaler.VelocityScaletoSIUnits() / GravityScaler.GetVelocityScale();
        //Debug.LogFormat("timeAtSOI={0} tliPhase={1} enterSoiPhase={2} moonPhase={3}", 
        //    timeatSoi, ghostShipOrbit[TLI].phase, ghostShipOrbit[ENTER_SOI].phase, moonPhase);
        ...
        Vector3d earthPos = ge.GetPositionDoubleV3(planet);
        ...
        //Debug.LogFormat("r_from={0}:{1} r_to={2}:{3}",
        //    departurePoint, GravityScaler.ScaleGEPositionToSI(departurePoint),
        //    soiEntryPos, GravityScaler.ScaleGEPositionToSI(soiEntryPos)
        //    );
        ...
        Vector3 shipPos = ge.GetPhysicsPosition(spaceship) - earthPos.ToVector3();
        Vector3 shipOrbitAxis = Vector3.Cross(ge.GetVelocity(spaceship), shipPos).normalized;
        ...
        // DEBUG
        //Vector3 moonV = ge.GetVelocity(ghostMoon[MOON_SOI_ENTER]);
        //Debug.LogFormat("tli_vel={0}:{1} tli_final={2}:{3} moonV={4}:{5} t_flight={6}",
        //    lambertB.GetTransferVelocity(), lambertB.GetTransferVelocity() * velFactor,
        //    lambertB.GetFinalVelocity(), lambertB.GetFinalVelocity() * velFactor,
        //    moonV, moonV * (float)velFactor, t_flight);

        ghostShipOrbit[TO_MOON].InitFromRVT(departurePoint, lambertB.GetTransferVelocity(), timeNow, planet, false);
        // Hack
        OrbitPredictor op = ghostShipOrbit[TO_MOON].GetComponentInChildren<OrbitPredictor>();
        op.UpdateOrbitU();
        ...
        // Debug.LogFormat("hyperTOF={0} soiE={1} soiExit={2}", hyperTOF, soiEnterRelative, soiExitRelative);
