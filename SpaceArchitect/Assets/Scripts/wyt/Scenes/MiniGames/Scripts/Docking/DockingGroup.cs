...
public class DockingGroup : MonoBehaviour {
    ...
    private float dockingModeRadius = 5.0f;
    ...
    private void Start() {
        ...
        for (int i=0; i < nbodies.Length; i++) {
            ...
            rigidBodies[i] = nbodies[i].GetComponentInChildren<Rigidbody>();
            if (rigidBodies[i] == null) {
                Debug.LogError("Abort - No rigidbody detected on " + nbodies[i].gameObject.name);
                return;
            }
        }
    ...
    void Update() {
        // assume active ship is first
        NBody nbody = nbodies[0];
        if (state != State.FREE) {
            nbody = cmNbody;
        }
        rcs[0].SetupFrame(ge.GetPhysicsPosition(nbody), ge.GetVelocity(nbody));
        float d = Vector3.Magnitude(rigidBodies[0].transform.position - rigidBodies[1].transform.position);
	...
	private void Activate() {
        ...
        foreach (NBody nbody in nbodies) {
            ...
            vel = GravityScaler.ScaleVelSceneToPhys(vel - cmVel);
