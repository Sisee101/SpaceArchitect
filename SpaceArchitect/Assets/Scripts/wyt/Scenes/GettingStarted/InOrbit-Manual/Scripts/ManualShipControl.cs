...
public class ManualShipControl : MonoBehaviour {
    ...
    /// <summary>
    /// Based on the selected co-ordinate frame, assign the vectors for the principle axes. 
    /// </summary>
    private void SetAxes() {
        ...
        switch(velocityFrame) {
            ...
            case Frame.ORBIT:
                ...
                Vector3 vel = ge.GetVelocity(shipNbody) - ge.GetVelocity(orbitCenter);
                axes[2] = Vector3.Normalize(Vector3.Cross(axes[0], vel));
        ...
        // Debug.LogFormat("axes 0={0} 1={1} 2={2}", axes[0], axes[1], axes[2]);
    }
    ...
    public void SetOrbitCenter(NBody nbody)
    {
        orbitCenter = nbody;
    }
    ...
    private void SetState(UIState newState) {
        ...
        switch (uiState) {
            ...
            case UIState.IDLE:
                ...
                if (pauseOnManeuver && ge.GetEvolve()) {
                    ge.SetEvolve(false);
                }
                shipVelocity = ge.GetVelocityDoubleV3(shipNbody);
                break;
