...
public class ManualSceneController : MonoBehaviour
{
    ...
    [SerializeField]
    private bool pauseOnManeuver = true;
    ...
    public void SetSpaceship( NBody spaceship)
    {
    ...
    private void SetState(State newState) {
        ...
        switch(newState)
        {
            ...
            case State.SET_MANUEVER:
                ...
                // set the orbit point to match the spaceship (so it cannot be mis-wired)
                OrbitPoint orbitPoint = shipAtOrbitPoint.GetComponent<OrbitPoint>();
                orbitPoint.orbitPredictor = spaceship.GetComponentInChildren<OrbitPredictor>();
                ge.AddBody(shipAtOrbitPoint);
                // orbitPoint.DoUpdate(false);
                // setting ship control active will pause the scene
            ...
            case State.EVOLVE_TO_MANEUVER:
                shipControl.SetActive(false);
                OrbitPoint orbitPoint2 = shipAtOrbitPoint.GetComponent<OrbitPoint>();
                orbitPoint2.enabled = false;
