...
public class DisplayManeuvers : MonoBehaviour
{
    ...
    public void Display(List<Maneuver> maneuvers) {
        ...
        for (int i=0; i < maneuvers.Count; i++) {
            ...
            nbodies[i].initialPos = pos;
            // add to GE??
            if (!addedToGE[i]) {
                ge.AddBody(markers[i]);
                addedToGE[i] = true;
                orbitPredictors[i].Init();
            }
            // ugh. Since markers may be fixed objects, need to set their positions there too.
            // This is because on evolve they retreive their fixed position each time. 
            FixedObject fo = nbodies[i].GetComponent<FixedObject>();
            if (fo != null) {
                // A fixed object assumes the initial transform position is unmapped. The Start()
                // will take this value.
                markers[i].transform.position = pos;
                fo.SetPositionDouble(pos3d);
            }
            else {
                ge.SetPositionDoubleV3(nbodies[i], pos3d);
            }
