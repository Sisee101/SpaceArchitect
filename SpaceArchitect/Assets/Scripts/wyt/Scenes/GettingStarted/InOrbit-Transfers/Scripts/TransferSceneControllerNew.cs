...
public class TransferSceneControllerNew : MonoBehaviour
{
    [SerializeField]
    ...
    [SerializeField]
    [Tooltip("Start scene with Maneuver pressed.")]
    private bool maneuverAtStart = false;
    ...
    // Need to buffer the key press from Update and pass over to FixedUpdate.
    // If check in FixedUpdate every once in a while will get a double press when two FixedFrames
    // run in between two Updates().
    private KeyCode lastKey = KeyCode.None;
    private KeyCode[] codes = new KeyCode[] { KeyCode.A, KeyCode.X, KeyCode.M, KeyCode.P, KeyCode.U };
    ...
    private void ActivateGhostShip() {
        if (shipAtOrbitPoint.activeInHierarchy) {
            Debug.LogWarning("ship already active. skip");
        }
        ...
        if (destinationOrbitPoint != null) {
            ...
            destinationOrbitPoint.DoUpdate(false);
        }
    }
    ...
    private void RecordKeyboardInput()
    {
        if (lastKey != KeyCode.None)
            return;
        if (maneuverAtStart && !maneuverStarted) {
            lastKey = KeyCode.M;
    ...
    private bool HandleKeyboardInput(OrbitPoint orbitPoint, KeyCode keyPressed) {
        if (keyPressed == KeyCode.M) {
            if (state == State.IDLE) {
                switch (transferShip.GetTransferType()) {
                    ...
                    default:
                        Debug.LogWarning("Unsupported transfer:" + transferShip.GetTransferType());
                        break;
                }
            } else {
                SetState(State.IDLE);
            ...
            maneuvers[maneuvers.Count - 1].onExecuted = ManeuverExecuted;
            // switched maneuvers to actual spaceship, cannot use transferShip on ghost
    ...
    // Can get glitching if check input on the FixedUpdate loop.
    void Update()
    {
        RecordKeyboardInput();
    }

    // When running continuously, need to sync this to the GE updates, otherwise the maneuver display jitters. 
    // May be a startup issue with orbit point on first frame if start state is not IDLE. 
    void FixedUpdate()
    {
        if (lastKey != KeyCode.None) {
            HandleKeyboardInput(shipOrbitPoint, lastKey);
            lastKey = KeyCode.None;
            return; // Let GE chew on changes for this cycle, otherwise will get NPE below on 1st M
        }
            ...
            case State.SET_LAMBERT_PO:
                ...
                if (destinationOrbitPoint != null) {
                    if (transferShip.GetTransferType() == TransferShip.Transfer.LAMBERT_POINT) {
                        transferShip.SetTargetPoint(destinationOrbitPoint.GetPositionDoubleV3());
                    } else {
