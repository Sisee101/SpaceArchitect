...
public class AddDeleteTester : MonoBehaviour {
    ...
    private void OrbitUInit(GameObject go, NBody nbody, GameObject parent, string bodyType) {

        OrbitUniversal ou = go.GetComponent<OrbitUniversal>();
        ou.centerNbody = parent.GetComponent<NBody>();
        if (ou == null) {
            Debug.LogError("Failed to get OrbitUniversal from prefab:" + go.name);
            return;
        }
        // Awkward test code
        if (parent == star) {
            ou.eccentricity = Random.Range(minEccentricity, maxEccentricity);
            ou.SetMajorAxisInspector( Random.Range(minRadius, maxRadius));
            ou.inclination = Random.Range(-80f, 80f);
        } else {
            // moon test, so keep "a" small
            ou.eccentricity = 0;
            ou.SetMajorAxisInspector( moonRadius);
            ou.inclination = Random.Range(-80f, 80f);
        }
        if (bodyType == KEPLER) {
            ou.evolveMode = OrbitUniversal.EvolveMode.KEPLERS_EQN;
        }
        ou.Init();
    ...
    private void BinaryPlanetInit(GameObject go, NBody nbody, GameObject parent)
    {
        // Init both the OrbitU (the orbit of the pair around star) and the binary
        OrbitUniversal[] orbits = nbody.GetComponents<OrbitUniversal>();
        foreach(OrbitUniversal ou in orbits) {
            // the parent has a BinaryOrbit and an OrbitUniversal. The binary sets the params of the child objects
            if (ou.GetType() == typeof(BinaryOrbit)) {
                ou.eccentricity = Random.Range(minEccentricity, maxEccentricity);
                // scale down orbit
                float scale = 0.01f;
                ou.SetMajorAxisInspector(Random.Range(scale * minRadius, scale * maxRadius));
                ou.inclination = Random.Range(-80f, 80f);
            } else {
                // this is the orbit for the center of mass of the binary
                ou.centerNbody = parent.GetComponent<NBody>();
                ou.eccentricity = Random.Range(minEccentricity, maxEccentricity);
                ou.SetMajorAxisInspector(Random.Range(minRadius, maxRadius));
                ou.inclination = Random.Range(-80f, 80f);
    ...
    private void AddBodyToParent(string bodyType, GameObject parent) {
        ...
        if (bodyType == HYPER) {
            ...
            BinaryPlanetInit(go, nbody, parent);
        } else if (bodyType != FIXED) {
            // this sets Kepler if needed
            OrbitUInit(go, nbody, parent, bodyType);
        }
    ...
    public void RemoveAll(string bodyType)
    {
        List<GameObject> bodyList = ListForType(bodyType);
        int count = bodyList.Count;
        for (int i=0; i < count; i++)
            RemoveEntryFromList(0, bodyList, bodyType);
    }

    private List<GameObject> ListForType(string bodyType)
    {
    ...
    private void RemoveEntryFromList(int entry, List<GameObject> bodyList, string bodyType)
    {
     ...
     public void RemoveBody(string bodyType) {
		...
		if (bodyList.Count > 0) {
            ...
            RemoveEntryFromList(entry, bodyList, bodyType);
		} else {
			Debug.Log("All objects of that type removed. " + bodyType);
		}
	}
